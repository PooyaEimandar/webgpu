...
fn vs_main(input: VertexInput) -> VertexOutput {
    ...
    let light_position = vec3<f32>(0.0, 0.0, 4.8);
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let diffuse = max(dot(normal, light), 0.35);
    let specular = pow(max(dot(reflected, view), 0.0), 28.0) * color.a;
    let distance_fade = clamp(1.15 - length(input.view_vec) * 0.022, 0.28, 1.0);
    let lit = (color.rgb * (0.2 + diffuse * 0.95) + vec3<f32>(specular)) * distance_fade;
