...
@group(0) @binding(2) var base_color_textures: texture_2d_array<f32>;
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    let material = materials[min(input.material, arrayLength(&materials) - 1u)];
    let layer = i32(max(0.0, material.params.y));
    let texel = textureSample(base_color_textures, base_color_sampler, input.uv, layer);
    // Alpha-mask cutout (foliage) when the material requests it.
    if material.params.z > 0.5 && texel.a * material.base_color.a < material.params.w {
        discard;
    }
    ...
    let emission = material.emission_roughness.rgb;
    return vec4<f32>(direct * shadow + punctual + ambient + emission, 1.0);
}
