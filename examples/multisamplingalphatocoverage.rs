...
const OAKTREE_GLTF_URL: &str = "assets/models/oaktree.gltf";
...
impl InstanceData {
    const ATTRIBUTES: [wgpu::VertexAttribute; 1] = wgpu::vertex_attr_array![4 => Float32x3];
...
struct AlphaToCoverageExample {
    ...
    assets: Option<AlphaToCoverageAssets>,
}

impl AlphaToCoverageExample {
    fn new(assets: AlphaToCoverageAssets) -> Self {
        Self {
            assets: Some(assets),
    ...
    fn initial_camera() -> FpsCamera {
        FpsCamera::new(glam::Vec3::new(0.0, 1.0, 4.0), 0.0, 0.0)
    }
...
impl Example for AlphaToCoverageExample {
    fn settings(&self) -> ExampleSettings {
        ExampleSettings {
            title: "Multisampling alpha to coverage".to_owned(),
            ..Default::default()
        }
    }
    ...
    fn init(&mut self, context: &mut RenderContext) -> RenderResult<()> {
        let assets = self.assets.take().ok_or_else(|| {
            RenderError::message("multisampling alpha-to-coverage assets were not loaded")
        })?;
        self.gpu_device_info = context.gpu_device_info();

        let shader = shader::wgsl_module(
            &context.device,
            Some("multisampling alpha to coverage shader"),
            include_str!("../shaders/multisamplingalphatocoverage.wgsl"),
        );
        let uniform_bind_group_layout =
            context
                .device
                .create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                    label: Some("multisampling alpha to coverage uniform layout"),
        ...
        let material_bind_group_layout =
            context
                .device
                .create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                    label: Some("multisampling alpha to coverage material layout"),
                    entries: &[
        ...
        let pipeline_layout =
            context
                ...
                .create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                    label: Some("multisampling alpha to coverage pipeline layout"),
                    bind_group_layouts: &[
        ...
        self.pipeline = Some(context.device.create_render_pipeline(
            &wgpu::RenderPipelineDescriptor {
                label: Some("multisampling alpha to coverage pipeline"),
                layout: Some(&pipeline_layout),
                vertex: wgpu::VertexState {
                    module: &shader,
                    entry_point: Some("vs_main"),
                    compilation_options: Default::default(),
                    buffers: &[TreeVertex::layout(), InstanceData::layout()],
                },
        ...
        let uniform_buffer = buffer::uniform_buffer(
            &context.device,
            Some("multisampling alpha to coverage uniforms"),
            &uniforms,
        );
        self.uniform_bind_group = Some(context.device.create_bind_group(
            &wgpu::BindGroupDescriptor {
                label: Some("multisampling alpha to coverage uniform bind group"),
        ...
        self.vertex_buffer = Some(buffer::vertex_buffer(
            &context.device,
            Some("multisampling alpha to coverage tree vertices"),
            &assets.mesh.vertices,
        ));
        self.index_buffer = Some(buffer::index_buffer(
            &context.device,
            Some("multisampling alpha to coverage tree indices"),
            &assets.mesh.indices,
        ));
        let instances = tree_instances();
        self.instance_count = instances.len() as u32;
        self.instance_buffer = Some(buffer::vertex_buffer(
            &context.device,
            Some("multisampling alpha to coverage instances"),
            &instances,
        ));
    ...
    fn render(
        ...
        self.overlay
            .as_mut()
            .ok_or_else(|| {
                RenderError::message("multisampling alpha to coverage text overlay initialized")
            })?
            .prepare(context)?;
        self.joystick_overlay
            .as_mut()
            .ok_or_else(|| {
                RenderError::message("multisampling alpha to coverage joystick overlay initialized")
            })?
            .prepare(context, &self.joystick)?;

        let pipeline = self.pipeline.as_ref().ok_or_else(|| {
            RenderError::message("multisampling alpha to coverage pipeline initialized")
        })?;
        let uniform_bind_group = self.uniform_bind_group.as_ref().ok_or_else(|| {
            RenderError::message("multisampling alpha to coverage uniform bind group initialized")
        })?;
        let vertex_buffer = self.vertex_buffer.as_ref().ok_or_else(|| {
            RenderError::message("multisampling alpha to coverage vertex buffer initialized")
        })?;
        let index_buffer = self.index_buffer.as_ref().ok_or_else(|| {
            RenderError::message("multisampling alpha to coverage index buffer initialized")
        })?;
        let instance_buffer = self.instance_buffer.as_ref().ok_or_else(|| {
            RenderError::message("multisampling alpha to coverage instance buffer initialized")
        })?;
        let msaa_targets = self.msaa_targets.as_ref().ok_or_else(|| {
            RenderError::message("multisampling alpha to coverage targets initialized")
        })?;

        {
            let mut pass = encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                label: Some("multisampling alpha to coverage render pass"),
            ...
            for range in &self.draw_ranges {
                let Some(material) = self.material_textures.get(range.material_index) else {
                    return Err(RenderError::message(format!(
                        "multisampling alpha to coverage material {} is missing",
...

        {
            let mut pass = render_pass::begin_color_load(
                encoder,
                Some("multisampling alpha to coverage text pass"),
                view,
            );
            self.overlay
                .as_ref()
            ...
            self.joystick_overlay
                ...
                .ok_or_else(|| {
                    RenderError::message(
                        "multisampling alpha to coverage joystick overlay initialized",
                    )
                })?
                .render(&mut pass);
        }
...
fn create_msaa_targets(context: &RenderContext) -> MsaaTargets {
    ...
    let color = context.device.create_texture(&wgpu::TextureDescriptor {
        label: Some("multisampling alpha to coverage color target"),
        size,
        mip_level_count: 1,
    ...
    let depth = context.device.create_texture(&wgpu::TextureDescriptor {
        label: Some("multisampling alpha to coverage depth target"),
        size,
        mip_level_count: 1,
        sample_count: MSAA_SAMPLE_COUNT,
        dimension: wgpu::TextureDimension::D2,
    ...
    for material in materials {
        let image = images.get(material.image_index).ok_or_else(|| {
            RenderError::message(format!(
                "multisampling alpha to coverage material image {} is missing",
        ...
        let gpu_texture = texture::Texture::from_rgba8_2d_with_sampler(
            &context.device,
            &context.queue,
            Some("multisampling alpha to coverage material texture"),
            image,
        ...
        let bind_group = context
            ...
            .create_bind_group(&wgpu::BindGroupDescriptor {
                label: Some("multisampling alpha to coverage material bind group"),
                layout,
                entries: &[
                    wgpu::BindGroupEntry {
                        binding: 0,
...
fn load_alpha_to_coverage_assets_from_bytes(
    gltf_bytes: &[u8],
    loader: &AssetLoader,
) -> RenderResult<AlphaToCoverageAssets> {
    let gltf = gltf::Gltf::from_slice(gltf_bytes)
        .map_err(|error| RenderError::message(format!("failed to parse oaktree.gltf: {error}")))?;
    let buffers = decode_gltf_buffers(&gltf, "oaktree.gltf")?;
    let mut images = decode_gltf_images(&gltf, &buffers, loader, "oaktree.gltf")?;
...
fn load_tree_mesh(
    ...
    buffers: &[Vec<u8>],
    materials: &[MaterialInfo],
) -> RenderResult<TreeMesh> {
    let scene = gltf
        .default_scene()
        .or_else(|| gltf.scenes().next())
        .ok_or_else(|| RenderError::message("oaktree.gltf has no scene"))?;
    ...
    if vertices.is_empty() {
        return Err(RenderError::message("oaktree.gltf has no vertices"));
    }
    if indices.is_empty() {
        return Err(RenderError::message("oaktree.gltf has no indices"));
    }

    let vertex_count = vertices.len() as u32;
    if let Some(index) = indices.iter().copied().find(|index| *index >= vertex_count) {
        return Err(RenderError::message(format!(
            "oaktree.gltf index {index} is outside vertex count {vertex_count}"
        )));
    }
    ...
    for image in gltf.images() {
        let bytes = match image.source() {
            gltf::image::Source::Uri { uri, .. } => decode_data_uri(uri)?.ok_or_else(|| {
                RenderError::message(format!(
                    "{label} uses external image {uri}; this example expects embedded images"
                ))
            })?,
        ...
        let name = image.name().map_or("embedded glTF image", |value| value);
...
#[cfg(not(target_arch = "wasm32"))]
fn load_alpha_to_coverage_assets() -> RenderResult<AlphaToCoverageAssets> {
    let loader = AssetLoader::new();
    let gltf_bytes = loader.fetch_url_bytes(OAKTREE_GLTF_URL)?;
    load_alpha_to_coverage_assets_from_bytes(&gltf_bytes, &loader)
}

#[cfg(target_arch = "wasm32")]
async fn load_alpha_to_coverage_assets() -> RenderResult<AlphaToCoverageAssets> {
    let loader = AssetLoader::new();
    let gltf_bytes = loader.fetch_url_bytes(OAKTREE_GLTF_URL).await?;
    load_alpha_to_coverage_assets_from_bytes(&gltf_bytes, &loader)
}

#[cfg(not(target_arch = "wasm32"))]
fn main() -> RenderResult<()> {
    sib::render::run(AlphaToCoverageExample::new(load_alpha_to_coverage_assets()?))
}
...
pub fn start() -> Result<(), wasm_bindgen::JsValue> {
    wasm_bindgen_futures::spawn_local(async {
        match load_alpha_to_coverage_assets().await {
            Ok(assets) => {
                if let Err(error) = sib::render::run(AlphaToCoverageExample::new(assets)) {
